...
//
    ...
    public function goodsSelTest(){
        $a=BaseRepository::getModel('\app\shop\model\GoodsModel');
        $res=$a->mySelect();
//        $goodsModel=new GoodsModel();
...
//        $res=$this->goodsModel->myInsertGetId($data);
    ...
    public function ruleTest(){
        $data=input('post.');
//        $test=Loader::validate('Goods');
        $test=\think\loader::validate('Goods');

//        $test=validate('test');
        $result=$test->check(['username'=>$data['username'],'password'=>$data['password']]);
//        return $result;
        return msg('400','',$result->getError());
    }
}